...
Complete working support for the reBot Arm **B601-RS**: kinematics, collision geometry, 3D models and
the parallel gripper on the RobStride variant, plus the arm frame correction that bench-testing it turned up.

### Added
- The B601-RS serves its own kinematic model: a bundled RS URDF (Seeed `reBotArm_control_py` `urdf/RS` @ `76512ea`),
  RS collision boxes and decimated meshes, and per-link GLBs coloured from the URDF materials. `get_end_position`,
  `get_kinematics`, `get_geometries`, `Get3DModels` and motion-service `move_to_position` describe the RS arm.
...
- The gripper component runs on the B601-RS. `variant: "rs"` drives motor `0x07` (a RobStride rs-00) in
  ...
  than 1440 deg. Configure reads the jaw's position and commands exactly that before enabling torque, so
  the motor holds where it already is rather than resuming the internal setpoint a RobStride in profile
  position keeps across a restart, which after a restart is stale.
- **RS gripper speed is the module pacing the setpoints, as the arm already does, and not any motor
  parameter or command field.** The gripper interpolates a move with the same `trajectory.plan` the arm
  uses and streams the result one setpoint per tick at 20 Hz, reading the jaw's position between ticks.
  ...
  frame, since `Mode.POS_VEL` already is native mode 2 (PP), but `vel_max` is a per-hop ceiling at
  twice the paced speed rather than the speed knob: enough headroom to reach the next setpoint within a
  tick whether or not the firmware honours the field. `acc_set` is `vel_max / 0.1 s`. Neither is an
  attribute. `move_timeout_s` on RS is slack past the planned duration of the move rather than the
  whole budget, because at 10 deg/s a full-travel move legitimately takes half a minute.
  Position samples during a move, and in `stop()`, come from the `mechPos` parameter read (RID `0x7019`),
  a request/response round trip that is current by construction: the streamed RobStride status frame runs
  about half a second behind the motor, so a move in flight would poll the same angle repeatedly and read
  as a stalled jaw. Readiness, health and status reads still use the streamed frame, which is the only one
  carrying faults and temperatures, and a `mechPos` read that times out counts as a missing sample rather
  than a still jaw, so a silent motor cannot look like a stall.
...
- New RS attribute `grip_current_a`, default 1.0: an absolute grip current cap in amps, written to the
  ...
  cardboard box and holds it without crushing it. The configure log reports the motor's own limit
  so it can be retuned from evidence.
  The RID is inferred, not proven. Seeed's stack confirms only its neighbours in the standard RobStride
  parameter table (`0x7017` limit_spd, `0x701E`-`0x7020` the gains) and this module already uses `0x7019`
  mechPos; motorbridge's native library lists `limit_cur` in the same position. So the module reads the
  value back after writing and logs both, warns if they disagree, and carries on uncapped rather than
  failing to build if the parameter does not exist.
...
- `tests/smoke_hardware.py --variant rs --port <chan> --gripper` jogs motor `0x07` by a signed step, with
  ...
  `open_position_deg`. Each step prints both reads -- the streamed frame and the `mechPos` parameter
  read, each with the time since the command was sent, plus a second `mechPos` read half a second later.
  A `param` column that tracks the target while `stream` trails it means the read is stale; both trailing
  together, with `later` still climbing, means the motor is genuinely slow.
...
- The end pose `get_end_position` reports, and that motion-service targets are expressed in, is the
  tool mount. On DM it moves back 155 mm from the mount plate **and rotates**: at the zero pose DM now
  reads x 104.9, z 191.7 mm with orientation vector (1, 0, 0) and theta -180, where the mount plate read
  (0, 0, 1) and theta 0. The tool mount follows the +Z-as-approach-axis convention and the mount plate
  did not, so an existing DM pose target has to be re-expressed, not merely shifted, and taught poses
  have to be recaptured. RS reports the same frame: x 135.5, z 217.7 mm, (1, 0, 0) theta -180 at the
  zero pose.
...
- Settling is judged by variant: DM by near-zero velocity, RS by the position not changing *and* the
  ...
  measured half is only available because the module streams the move, and it is what tells a
  stalled jaw from a slow one, which "the position stopped changing" on its own cannot.
  An RS move that ends short of its target re-commands the angle the jaws actually reached, so the motor
  is not left driving at a target it cannot reach and grinding into the object until it faults. That
  re-command only fires when the jaw was seen to move and then stop: with no poll ever showing movement
  the reading is the suspect part, not the jaw, so the move logs a warning naming stale or unresponsive
  feedback and lets the setpoints stop where they are. DM is unchanged there: `FORCE_POS` caps the
  current by design and that standing push is how a DM grab keeps its grip.
...
- `spatial` exposes `MODELS["dm"|"rs"]` and the FK, gravity and asset helpers live on the model; the
  DM-only module-level names are gone.
- A model with no collision primitives logs a warning at configure.

### Removed
- `include_gripper_geometry`. It could not survive the trim to the tool mount: the RDK keeps only the
  first `<collision>` per link and `link6` already has its own, so the gripper geometry would have been
  discarded with no warning. A config that still carries the attribute is ignored, with a warning at
  configure. A tool with no gripper component of its own is now described by a `geometry` in the arm's
  frame config, which is the idiomatic Viam answer.
...
- The gripper's geometry and kinematics show up in the app's 3D scene and in the frame system at all.
  ...
  gripper never loaded (`buildCache # of frames: 2`, and nothing else in the log to say why). The
  gripper's model has had that shape since it was written, and nothing exercised it until now. The
  model is now one chain, `tool_mount` -> `gripper_base` -> `finger_left_link`, and the right finger's
  static travel envelope is unioned into `gripper_base`'s collision box (DM 107 x 201 x 68 mm, RS
  157 x 198 x 82 mm) rather than dropped, since a planner blind to half the jaw is worse than a coarse
  one. A link gets exactly one `<collision>` -- the RDK keeps the first and discards the rest in
  silence -- so the union is the only way to carry that volume, and it is why `gripper_base` now serves
  a box even in `meshes` mode, leaving the left finger's STL as the only mesh served. `get_geometries`
  is unchanged and still reports all three parts at their real sizes; the served model is deliberately
  the coarser of the two.
- The gripper component's frame in world is aligned with the arm's. Every joint of the served gripper
  model carried its vendor rotation -- the mount plate's on `tool_mount_joint`, the finger's on the
  finger joint -- so each link frame sat at some arbitrary angle to the mount it hangs from: at the zero
  ...
  `tests/test_kinematics.py::test_gripper_boxes_did_not_move_in_space`. `tests/test_dm_baseline.py`
  re-pins the DM gripper payload hashes and the smaller served mesh set for both of these; the arm
  payloads, the gripper mesh bytes and the GLBs did not move.
...
- The DM meshes no longer arrive as floating shards. `tools/build_assets.py` decimated each part as one
  mesh, and because the source STLs store unshared vertices there was no edge to collapse, so the
  simplifier deleted triangles instead: `meshes/base_link.stl` was 3062 faces in 2409 loose pieces whose
  largest was 7 triangles. Decimation now welds vertices, works shell by shell, keeps a shell whole
  rather than cutting it below 48 faces, and drops the source's 1- and 2-triangle slivers. `base_link.stl`
  is now 2949 faces in 59 shells, the largest 306, and the worst remaining DM collision mesh is 58 shells
  against 2958 faces. The RS meshes are built the same way. Every DM mesh byte therefore moved, so
  `tests/test_dm_baseline.py` re-pins its mesh hashes and GLB sizes; the rebuild left the served URDF
  payloads alone, since the URDF names mesh files and does not embed their bytes.
- The GLB byte budget assumed 28 bytes per face where the real figure is about 18, so every visual model
  stopped at ~10,970 faces and left a third of the 300 KB cap unspent. The DM visual models now carry
  15,992-16,779 faces at up to 299 KB and the RS models 15,455-16,998 faces at 280-300 KB. The caps
  themselves are unchanged.

### Not yet on RS
- Gravity compensation in manual mode: the RS model exists and `{"gravity_torques": true}` reports its
  torques, but nothing is applied, so manual mode on RS is damping only. The bench check is
  `tests/smoke_hardware.py --gravity-check`.
...
- DM finger travel is this module's own 0.05 m, while Seeed's DM vendor URDF says 0.0285 per finger. DM
  is left unchanged pending the same caliper measurement (`kinematics.FINGER_TRAVEL_M` records the
  discrepancy).
